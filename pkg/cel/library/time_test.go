...
// evalTime compiles and evaluates expr with `time` fixed at now, returning
// the result and the TimeVal (for flip inspection).
...
func TestAddingTwoTimestampsErrors(t *testing.T) {
	env := timeEnv(t)
	ast, iss := env.Compile(`time.now() + timestamp(schema.t)`)
	if iss != nil && iss.Err() != nil {
		// Statically rejected is also acceptable (no ts+ts overload in CEL).
		return
	}
	prog, err := env.Program(ast)
	if err != nil {
	...
	_, _, err = prog.Eval(map[string]any{
		TimeVarName: NewTimeValue(time.Now()),
		"schema":    map[string]any{"t": "2026-01-01T00:00:00Z"},
	})
	if err == nil {
