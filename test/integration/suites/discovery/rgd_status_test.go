...
// a live RGD must not wedge its instances to ERROR. Before the fix, the status
// patch node was type-checked against a stale cached CRD schema, so the new
// field was "schema not found" and every instance (new and pre-existing) froze
// in ERROR. The fix types the status node against the in-memory RGD-derived
// schema, so both the existing instance and a new one converge with the added
// field.
